...
const HoverDropdown: React.FC<HoverDropdownProps> = ({ label, items }) => {
  ...
  return (
    ...
    >
      {/* Button / Trigger */}
      <button
        className={`flex items-center gap-1 whitespace-nowrap text-base hover:text-yellow-400 focus:outline-none transition-colors duration-150 ${open ? 'text-yellow-400' : ''}`} aria-haspopup="true" aria-expanded={open}
        >
        <span>{label}</span>
